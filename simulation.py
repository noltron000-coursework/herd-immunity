...
class Simulation(object):
	...
	def __init__(self, pop_size, vacc_percentage, initial_infected=1, virus):
		...
		The `next_person_id` is the next available id for all created `Person` objects, and should have a unique `_id` value.
		...
		# At the end of each time step, call `self._infect_newly_infected()` and then reset `.newly_infected` back to an empty list.
		self.logger = None
	...
	def run(self):
		...
		# TODO:
		# Finish this method.
		# To simplify the logic here, use the helper method `_simulation_should_continue()` to tell us whether or not we should continue the simulation and run at least 1 more `time_step`.
	...
	def time_step(self):
		'''
		This method should contain all the logic for computing one time stepin the simulation.
	...
	def interaction(self, person, random_person):
		'''
		This method should be called any time two living people are selected for an interaction.
	...
	def _infect_newly_infected(self):
		...
		# TODO:
		# Once you have iterated through the entire list of `self.newly_infected`, remember to reset `self.newly_infected` back to an empty list.
		pass
