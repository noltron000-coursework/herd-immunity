...
'''
	...
	def create_population(self, initial_infected):
		...
		- Once all infected person objects are created, begins creating healthy person objects.
			...
			- If that number is smaller than self.vacc_percentage, new person object will be created with is_vaccinated set to True.  Otherwise, is_vaccinated will be set to False.
	...
	To simplify the logic here, we will use the helper method _simulation_should_continue() to tell us whether or not we should continue the simulation and run at least 1 more time_step.
	...
	def run(self):
		...
		while should_continue:
			...
			# At the end of each iteration of this loop, remember to rebind should_continue to another call of self._simulation_should_continue()!
			pass
